"use client";

import { useState } from "react";
import { PlusCircle, Edit2, Trash2, Loader2, Receipt, Calendar, CreditCard, Landmark, Paperclip, AlertCircle } from "lucide-react";
import DataTable from "@/components/data-table";
import Modal from "@/components/modal";
import {
  createExpense,
  updateExpense,
  deleteExpense,
} from "@/app/actions/finance";

interface ExpenseData {
  id: string;
  expenseNumber: string;
  amount: number;
  tax: number;
  date: Date;
  description: string | null;
  supplier: string | null;
  paymentMethod: string;
  bankAccountId: string | null;
  cashAccountId: string | null;
  expenseCategoryId: string;
  reference: string | null;
  attachment: string | null;
  notes: string | null;
  status: string;
  createdAt: Date;
  expenseCategory: {
    id: string;
    name: string;
    color?: string;
    icon?: string;
  };
  bankAccount: {
    id: string;
    name: string;
    bankName: string;
  } | null;
  cashAccount: {
    id: string;
    name: string;
  } | null;
}

interface CategoryData {
  id: string;
  name: string;
  color?: string;
  icon?: string;
  isActive: boolean;
}

interface BankAccountData {
  id: string;
  name: string;
  bankName: string;
}

interface CashAccountData {
  id: string;
  name: string;
}

interface ExpensesClientProps {
  initialExpenses: ExpenseData[];
  categories: CategoryData[];
  bankAccounts: BankAccountData[];
  cashAccounts: CashAccountData[];
}

export default function ExpensesClient({
  initialExpenses,
  categories,
  bankAccounts,
  cashAccounts,
}: ExpensesClientProps) {
  const [expenses, setExpenses] = useState<ExpenseData[]>(initialExpenses);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingExpense, setEditingExpense] = useState<ExpenseData | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formError, setFormError] = useState("");

  // Filters State
  const [categoryFilter, setCategoryFilter] = useState("ALL");
  const [methodFilter, setMethodFilter] = useState("ALL");

  // Form State
  const [formData, setFormData] = useState({
    amount: "",
    tax: "0",
    date: new Date().toISOString().split("T")[0],
    description: "",
    supplier: "",
    paymentMethod: "CASH",
    bankAccountId: "",
    cashAccountId: "",
    expenseCategoryId: "",
    reference: "",
    attachment: "",
    notes: "",
    status: "PAID",
  });

  const activeCategories = categories.filter(
    (c) => c.isActive || c.id === editingExpense?.expenseCategoryId
  );

  const handleOpenCreate = () => {
    setEditingExpense(null);
    setFormData({
      amount: "",
      tax: "0",
      date: new Date().toISOString().split("T")[0],
      description: "",
      supplier: "",
      paymentMethod: "CASH",
      bankAccountId: bankAccounts[0]?.id || "",
      cashAccountId: cashAccounts[0]?.id || "",
      expenseCategoryId: activeCategories[0]?.id || "",
      reference: "",
      attachment: "",
      notes: "",
      status: "PAID",
    });
    setFormError("");
    setIsModalOpen(true);
  };

  const handleOpenEdit = (expense: ExpenseData) => {
    setEditingExpense(expense);
    setFormData({
      amount: String(expense.amount),
      tax: String(expense.tax || 0),
      date: new Date(expense.date).toISOString().split("T")[0],
      description: expense.description || "",
      supplier: expense.supplier || "",
      paymentMethod: expense.paymentMethod,
      bankAccountId: expense.bankAccountId || bankAccounts[0]?.id || "",
      cashAccountId: expense.cashAccountId || cashAccounts[0]?.id || "",
      expenseCategoryId: expense.expenseCategoryId,
      reference: expense.reference || "",
      attachment: expense.attachment || "",
      notes: expense.notes || "",
      status: expense.status || "PAID",
    });
    setFormError("");
    setIsModalOpen(true);
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this expense? This will also automatically delete the linked Cash Book Entry and reverse any bank or cash ledger balance adjustments.")) {
      return;
    }
    const res = await deleteExpense(id);
    if (res.success) {
      setExpenses(expenses.filter((e) => e.id !== id));
    } else {
      alert(res.error || "Failed to delete expense");
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    setFormError("");

    const parsedAmount = parseFloat(formData.amount);
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      setFormError("Please enter a valid amount greater than 0.");
      setIsSubmitting(false);
      return;
    }

    const parsedTax = parseFloat(formData.tax);
    if (isNaN(parsedTax) || parsedTax < 0) {
      setFormError("Tax amount must be 0 or greater.");
      setIsSubmitting(false);
      return;
    }

    if (!formData.expenseCategoryId) {
      setFormError("Please select an expense category.");
      setIsSubmitting(false);
      return;
    }

    if (formData.paymentMethod === "BANK" && !formData.bankAccountId) {
      setFormError("Please select a bank account.");
      setIsSubmitting(false);
      return;
    }

    if (formData.paymentMethod === "CASH" && !formData.cashAccountId) {
      setFormError("Please select a cash account.");
      setIsSubmitting(false);
      return;
    }

    const payload = {
      amount: parsedAmount,
      tax: parsedTax,
      date: formData.date,
      description: formData.description.trim() || undefined,
      supplier: formData.supplier.trim() || undefined,
      paymentMethod: formData.paymentMethod,
      expenseCategoryId: formData.expenseCategoryId,
      bankAccountId: formData.paymentMethod === "BANK" ? formData.bankAccountId : undefined,
      cashAccountId: formData.paymentMethod === "CASH" ? formData.cashAccountId : undefined,
      reference: formData.reference.trim() || undefined,
      attachment: formData.attachment.trim() || undefined,
      notes: formData.notes.trim() || undefined,
      status: formData.status,
    };

    if (editingExpense) {
      const res = await updateExpense(editingExpense.id, payload);
      if (res.success && res.data) {
        const updatedCat = categories.find((c) => c.id === payload.expenseCategoryId);
        const updatedBank = payload.bankAccountId ? bankAccounts.find((b) => b.id === payload.bankAccountId) : null;
        const updatedCash = payload.cashAccountId ? cashAccounts.find((c) => c.id === payload.cashAccountId) : null;

        setExpenses(
          expenses.map((exp) =>
            exp.id === editingExpense.id
              ? {
                  ...exp,
                  amount: res.data.amount,
                  tax: res.data.tax,
                  date: new Date(res.data.date),
                  description: res.data.description,
                  supplier: res.data.supplier,
                  paymentMethod: res.data.paymentMethod,
                  bankAccountId: res.data.bankAccountId,
                  cashAccountId: res.data.cashAccountId,
                  expenseCategoryId: res.data.expenseCategoryId,
                  reference: res.data.reference,
                  attachment: res.data.attachment,
                  notes: res.data.notes,
                  status: res.data.status,
                  expenseCategory: {
                    id: payload.expenseCategoryId,
                    name: updatedCat?.name || "Unknown",
                  },
                  bankAccount: updatedBank
                    ? {
                        id: updatedBank.id,
                        name: updatedBank.name,
                        bankName: updatedBank.bankName,
                      }
                    : null,
                  cashAccount: updatedCash
                    ? {
                        id: updatedCash.id,
                        name: updatedCash.name,
                      }
                    : null,
                }
              : exp
          )
        );
        setIsModalOpen(false);
      } else {
        setFormError(res.error || "Failed to update expense.");
      }
    } else {
      const res = await createExpense(payload);
      if (res.success && res.data) {
        const createdCat = categories.find((c) => c.id === payload.expenseCategoryId);
        const createdBank = payload.bankAccountId ? bankAccounts.find((b) => b.id === payload.bankAccountId) : null;
        const createdCash = payload.cashAccountId ? cashAccounts.find((c) => c.id === payload.cashAccountId) : null;

        const newExpense: ExpenseData = {
          id: res.data.id,
          expenseNumber: res.data.expenseNumber,
          amount: res.data.amount,
          tax: res.data.tax,
          date: new Date(res.data.date),
          description: res.data.description,
          supplier: res.data.supplier,
          paymentMethod: res.data.paymentMethod,
          bankAccountId: res.data.bankAccountId,
          cashAccountId: res.data.cashAccountId,
          expenseCategoryId: res.data.expenseCategoryId,
          reference: res.data.reference,
          attachment: res.data.attachment,
          notes: res.data.notes,
          status: res.data.status,
          createdAt: res.data.createdAt,
          expenseCategory: {
            id: payload.expenseCategoryId,
            name: createdCat?.name || "Unknown",
          },
          bankAccount: createdBank
            ? {
                id: createdBank.id,
                name: createdBank.name,
                bankName: createdBank.bankName,
              }
            : null,
          cashAccount: createdCash
            ? {
                id: createdCash.id,
                name: createdCash.name,
              }
            : null,
        };
        setExpenses([newExpense, ...expenses]);
        setIsModalOpen(false);
      } else {
        setFormError(res.error || "Failed to record expense.");
      }
    }
    setIsSubmitting(false);
  };

  // Filter list
  const filteredExpenses = expenses.filter((e) => {
    const catMatch = categoryFilter === "ALL" || e.expenseCategoryId === categoryFilter;
    const methodMatch = methodFilter === "ALL" || e.paymentMethod === methodFilter;
    return catMatch && methodMatch;
  });

  const columns = [
    {
      header: "Expense No.",
      render: (row: ExpenseData) => (
        <span className="font-semibold text-white font-mono text-xs">{row.expenseNumber}</span>
      ),
    },
    {
      header: "Date",
      render: (row: ExpenseData) => (
        <div suppressHydrationWarning className="flex items-center gap-2 text-slate-300 font-sans text-xs">
          <Calendar className="h-3.5 w-3.5 text-slate-500" />
          {new Date(row.date).toLocaleDateString()}
        </div>
      ),
    },
    {
      header: "Category",
      render: (row: ExpenseData) => (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-violet-500/10 text-violet-300 border border-violet-500/10">
          <Receipt className="h-3 w-3" />
          {row.expenseCategory?.name || "Uncategorized"}
        </span>
      ),
    },
    {
      header: "Details",
      render: (row: ExpenseData) => (
        <div className="space-y-0.5">
          <p className="text-sm font-medium text-white max-w-[200px] truncate" title={row.description || ""}>
            {row.description || "No description"}
          </p>
          <div className="flex flex-wrap items-center gap-2">
            {row.supplier && (
              <span className="text-[10px] text-slate-400">Supplier: <b className="text-slate-300">{row.supplier}</b></span>
            )}
            {row.reference && (
              <span className="text-[10px] font-mono text-slate-500">Ref: {row.reference}</span>
            )}
            {row.attachment && (
              <span className="inline-flex items-center text-[10px] text-primary/80 gap-0.5" title={row.attachment}>
                <Paperclip className="h-2.5 w-2.5" /> File
              </span>
            )}
          </div>
        </div>
      ),
    },
    {
      header: "Payment Account",
      render: (row: ExpenseData) => {
        if (row.paymentMethod === "CASH") {
          return (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/10">
              <CreditCard className="h-3 w-3" /> CASH ({row.cashAccount?.name || "Cash"})
            </span>
          );
        } else {
          return (
            <div className="flex items-center gap-1 text-xs text-indigo-400 font-medium">
              <Landmark className="h-3.5 w-3.5" />
              <span className="truncate max-w-[120px]" title={row.bankAccount?.name || "Bank Account"}>
                {row.bankAccount?.name || "Bank Account"}
              </span>
            </div>
          );
        }
      },
    },
    {
      header: "Status",
      render: (row: ExpenseData) => (
        <span
          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
            row.status === "PAID"
              ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/10"
              : "bg-amber-500/10 text-amber-400 border-amber-500/10"
          }`}
        >
          {row.status === "PAID" ? "Paid" : "Pending"}
        </span>
      ),
    },
    {
      header: "Amount",
      render: (row: ExpenseData) => (
        <div>
          <span className="font-bold text-white text-base">${row.amount.toFixed(2)}</span>
          {row.tax > 0 && (
            <span className="text-[10px] text-slate-400 block mt-0.5">Tax: ${row.tax.toFixed(2)}</span>
          )}
        </div>
      ),
    },
    {
      header: "Actions",
      render: (row: ExpenseData) => (
        <div className="flex items-center gap-2">
          <button
            onClick={() => handleOpenEdit(row)}
            className="rounded-lg p-1.5 text-slate-400 hover:bg-white/5 hover:text-white transition cursor-pointer"
            title="Edit"
          >
            <Edit2 className="h-4 w-4" />
          </button>
          <button
            onClick={() => handleDelete(row.id)}
            className="rounded-lg p-1.5 text-red-400 hover:bg-red-500/10 hover:text-red-300 transition cursor-pointer"
            title="Delete"
          >
            <Trash2 className="h-4 w-4" />
          </button>
        </div>
      ),
    },
  ];

  return (
    <div className="space-y-6 font-sans">
      {/* Header section */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-white">Expenses</h2>
          <p className="text-slate-400 text-sm">Log, filter, and review all outgoing business expenses.</p>
        </div>
        <button
          onClick={handleOpenCreate}
          className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-primary/95 transition shadow-lg shadow-primary/20 cursor-pointer"
        >
          <PlusCircle className="h-4.5 w-4.5" />
          Log Expense
        </button>
      </div>

      {/* Filters block */}
      <div className="flex flex-wrap items-center gap-4 rounded-xl border border-white/5 bg-slate-900/10 p-4">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-400">Category:</span>
          <select
            value={categoryFilter}
            onChange={(e) => setCategoryFilter(e.target.value)}
            className="rounded-lg border border-white/10 bg-slate-950 px-3 py-1.5 text-xs text-white outline-none focus:border-primary cursor-pointer"
          >
            <option value="ALL">All Categories</option>
            {categories.map((c) => (
              <option key={c.id} value={c.id}>
                {c.name}
              </option>
            ))}
          </select>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-400">Method:</span>
          <select
            value={methodFilter}
            onChange={(e) => setMethodFilter(e.target.value)}
            className="rounded-lg border border-white/10 bg-slate-950 px-3 py-1.5 text-xs text-white outline-none focus:border-primary cursor-pointer"
          >
            <option value="ALL">All Methods</option>
            <option value="CASH">CASH Only</option>
            <option value="BANK">BANK Only</option>
          </select>
        </div>
      </div>

      {/* Expenses Table */}
      <div className="rounded-2xl border border-white/5 bg-slate-900/20 p-6 backdrop-blur-md shadow-xl">
        <DataTable
          data={filteredExpenses}
          columns={columns}
          searchKey="description"
          searchPlaceholder="Search description, suppliers, codes..."
          itemsPerPage={10}
          emptyState="No expenses logged matching criteria."
        />
      </div>

      {/* Modal Editor */}
      <Modal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        title={editingExpense ? `Edit Expense: ${editingExpense.expenseNumber}` : "Log New Expense"}
        className="max-w-lg"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          {formError && (
            <div className="rounded-xl bg-rose-500/10 border border-rose-500/10 p-3 text-xs text-rose-400 font-medium">
              {formError}
            </div>
          )}

          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Category *</label>
              <select
                required
                value={formData.expenseCategoryId}
                onChange={(e) => setFormData({ ...formData, expenseCategoryId: e.target.value })}
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary cursor-pointer"
              >
                {activeCategories.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name} {!c.isActive ? "(Inactive)" : ""}
                  </option>
                ))}
                {activeCategories.length === 0 && (
                  <option value="">No active categories defined!</option>
                )}
              </select>
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Date *</label>
              <input
                type="date"
                required
                value={formData.date}
                onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Amount ($) *</label>
              <input
                type="number"
                step="0.01"
                required
                value={formData.amount}
                onChange={(e) => setFormData({ ...formData, amount: e.target.value })}
                placeholder="0.00"
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Tax ($)</label>
              <input
                type="number"
                step="0.01"
                value={formData.tax}
                onChange={(e) => setFormData({ ...formData, tax: e.target.value })}
                placeholder="0.00"
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Supplier Name (Optional)</label>
              <input
                type="text"
                value={formData.supplier}
                onChange={(e) => setFormData({ ...formData, supplier: e.target.value })}
                placeholder="e.g. Amazon, local vendor"
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Reference / Invoice #</label>
              <input
                type="text"
                value={formData.reference}
                onChange={(e) => setFormData({ ...formData, reference: e.target.value })}
                placeholder="e.g. INV-998822"
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-400">Payment Method</label>
              <select
                value={formData.paymentMethod}
                onChange={(e) => setFormData({ ...formData, paymentMethod: e.target.value })}
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary cursor-pointer"
              >
                <option value="CASH">CASH (Physical Drawer)</option>
                <option value="BANK">BANK (Ledger Account)</option>
              </select>
            </div>

            {formData.paymentMethod === "BANK" ? (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-400">Bank Account *</label>
                <select
                  required={formData.paymentMethod === "BANK"}
                  value={formData.bankAccountId}
                  onChange={(e) => setFormData({ ...formData, bankAccountId: e.target.value })}
                  className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary cursor-pointer"
                >
                  {bankAccounts.map((b) => (
                    <option key={b.id} value={b.id}>
                      {b.name} ({b.bankName})
                    </option>
                  ))}
                  {bankAccounts.length === 0 && (
                    <option value="">No bank accounts defined!</option>
                  )}
                </select>
              </div>
            ) : (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-400">Cash Account *</label>
                <select
                  required={formData.paymentMethod === "CASH"}
                  value={formData.cashAccountId}
                  onChange={(e) => setFormData({ ...formData, cashAccountId: e.target.value })}
                  className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary cursor-pointer"
                >
                  {cashAccounts.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name}
                    </option>
                  ))}
                  {cashAccounts.length === 0 && (
                    <option value="">No cash accounts defined!</option>
                  )}
                </select>
              </div>
            )}

            <div className="space-y-1 col-span-2">
              <label className="text-xs font-semibold text-slate-400">Status</label>
              <select
                value={formData.status}
                onChange={(e) => setFormData({ ...formData, status: e.target.value })}
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary cursor-pointer"
              >
                <option value="PAID">PAID (Balances Adjusted & Logged)</option>
                <option value="PENDING">PENDING (Accrued / Accounts Payable)</option>
              </select>
            </div>

            <div className="space-y-1 col-span-2">
              <label className="text-xs font-semibold text-slate-400">Description</label>
              <textarea
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                placeholder="Log description..."
                rows={2}
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary resize-none"
              />
            </div>

            <div className="space-y-1 col-span-2">
              <label className="text-xs font-semibold text-slate-400 font-mono">Attachment URL / Path</label>
              <input
                type="text"
                value={formData.attachment}
                onChange={(e) => setFormData({ ...formData, attachment: e.target.value })}
                placeholder="e.g. receipts/invoice-url"
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary"
              />
            </div>

            <div className="space-y-1 col-span-2">
              <label className="text-xs font-semibold text-slate-400">Notes</label>
              <textarea
                value={formData.notes}
                onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                placeholder="Additional details..."
                rows={2}
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 px-4 text-sm text-white outline-none focus:border-primary resize-none"
              />
            </div>
          </div>

          <div className="flex justify-end gap-3 border-t border-white/5 pt-4">
            <button
              type="button"
              onClick={() => setIsModalOpen(false)}
              className="rounded-xl border border-white/10 bg-slate-900 hover:bg-slate-800 py-2.5 px-4 text-sm font-semibold text-white transition cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="rounded-xl bg-primary hover:bg-primary/95 py-2.5 px-5 text-sm font-semibold text-slate-950 transition flex items-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isSubmitting && <Loader2 className="h-4 w-4 animate-spin" />}
              {editingExpense ? "Save Changes" : "Log Expense"}
            </button>
          </div>
        </form>
      </Modal>
    </div>
  );
}
